// Enhanced YouTube Player with Zeroma Premium Design & Perfect Mobile Controls
// Optimized 2-Row Mobile Layout - All Buttons Always Visible
...
function onYouTubeIframeAPIReady() {
    console.log('🎓 Zeroma Premium YouTube API ready');
}
...
// Enhanced class card rendering with premium Zeroma design
...
// Show mobile video controls with enhanced visibility
function showMobileVideoControls() {
    ...
    if (mobileControls && !document.getElementById('video-player-container').classList.contains('hidden')) {
        ...
        // Auto-hide after 8 seconds (longer for better UX)
        setTimeout(() => {
        ...
        qualityItems.forEach(item => {
            const itemHandler = (e) => {
                ...
                if (quality === 'auto') {
                    ...
                    qualityBtn.textContent = e.target.textContent.replace(' HD', '');
                }
...
console.log('📱 Mobile optimizations:', isMobile ? 'Enabled with 2-row control layout' : 'Desktop mode');
console.log('🎮 Controls: Custom controls with full YouTube blocking active');
console.log('🎨 Design: Premium color scheme applied with perfect mobile UX');
